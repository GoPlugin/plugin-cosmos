...
fn transmit_happy_path() {
    ...
    // https://github.com/goplugin/plugin-cosmos/issues/19
...
fn revert_payouts_correctly() {
    ...
    // attempt to withdraw should fail without PLI token balance
    ...
    // attempt to change PLI token to trigger paying all oracles
...
fn set_billing_payout() {
    ...
    // oracles should be paid out (same as changing PLI token)
    for payee in env
        .transmitters
        .iter()
        .enumerate()
