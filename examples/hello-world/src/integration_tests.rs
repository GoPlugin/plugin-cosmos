...
mod mock {
    ...
    use plugin_cosmos::msg::QueryMsg as PluginQueryMsg;
    use plugin_cosmos::state::Round;
    ...
    pub fn contract() -> Box<dyn Contract<Empty>> {
        ...
        pub fn query(_deps: Deps, _env: Env, msg: PluginQueryMsg) -> StdResult<Binary> {
            match msg {
                PluginQueryMsg::Decimals {} => to_binary(&DECIMALS),
                PluginQueryMsg::LatestRoundData {} => to_binary(&ROUND),
                _ => unimplemented!(),
            }
        }
        let contract = ContractWrapper::new(execute, instantiate, query);
...
fn it_works() {
    ...
    // query round
    let round: plugin_cosmos::state::Round = env
        .router
        .wrap()
