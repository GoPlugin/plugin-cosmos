...
import (
	...
	relaylogger "github.com/goplugin/plugin-common/pkg/logger"

	"github.com/goplugin/plugin-cosmos/integration-tests/common"
	"github.com/goplugin/plugin-cosmos/integration-tests/gauntlet"
	...
	// "github.com/goplugin/plugin_latest/integration-testss/actions"
...
func TestOCRBasic(t *testing.T) {
	...
	nodeName := "primary"
	pluginClient, err := common.NewPluginClient(commonConfig.Env, commonConfig.ChainId, nodeName, commonConfig.NodeUrl, bech32Prefix)
	require.NoError(t, err, "Could not create plugin client")

	logger.Info().Str("node addresses", strings.Join(pluginClient.GetNodeAddresses(), " ")).Msg("Created plugin client")
	...
	for i, nodeAddr := range pluginClient.GetNodeAddresses() {
	...
	logger.Info().Str("address", linkTokenAddress).Msg("Deployed PLI token")
	os.Setenv("PLI", linkTokenAddress)
	...
	// Mint PLI tokens to aggregator
	_, err = cg.MintLinkToken(linkTokenAddress, ocrAddress, "100000000000000000000")
	require.NoError(t, err, "Could not mint PLI token")
	...
	cfg, err := pluginClient.LoadOCR2Config(proposalId)
	...
	p2pPort := "50200"
	err = pluginClient.CreateJobsForContract(
		commonConfig.ChainId,
		nodeName,
		p2pPort,
	...
	// 	//err = actions.TeardownSuite(t, t.Common.Env, utils.ProjectRoot, t.Cc.PluginNodes, nil, zapcore.ErrorLevel)
