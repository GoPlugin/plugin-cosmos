...
// TODO: those should be moved as a common part of plugin-testing-framework
...
func NewCommon(t *testing.T) *Common {
	...
	pluginConfig := fmt.Sprintf(`[[Cosmos]]
Enabled = true
ChainID = '%s'
Bech32Prefix = 'wasm'
GasToken = 'ucosm'
...
`, chainID, internalNodeUrl)
	log.Debug().Str("toml", pluginConfig).Msg("Created plugin config")
