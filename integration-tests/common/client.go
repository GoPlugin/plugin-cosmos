...
import (
	...
	"github.com/goplugin/plugin_latest/integration-testss/client"
	"github.com/goplugin/plugin_latest/core/services/job"
	"github.com/goplugin/plugin_latest/core/services/relay"
...
func NewPluginClient(env *environment.Environment, nodeName string, chainId string, tendermintURL string, bech32Prefix string) (*PluginClient, error) {
	nodes, err := connectPluginNodes(env)
	if err != nil {
		return nil, err
	}
	if len(nodes) == 0 {
...
func (cc *PluginClient) GetNodeAddresses() []string {
...
func (cc *PluginClient) LoadOCR2Config(proposalId string) (*OCR2Config, error) {
...
func (cc *PluginClient) CreateJobsForContract(chainId, nodeName, p2pPort, mockUrl string, juelsPerFeeCoinSource string, ocrControllerAddress string) error {
	...
	_, err := cc.PluginNodes[0].MustCreateJob(jobSpec)
	if err != nil {
		return err
	}
...
// connectPluginNodes creates a plugin client for each node in the environment
// This is a non k8s version of the function in plugin_k8s.go
// https://github.com/goplugin/blob/cosmos-test-keys/integration-tests/client/plugin_k8s.go#L77
func connectPluginNodes(e *environment.Environment) ([]*client.PluginClient, error) {
	var clients []*client.PluginClient
	for _, nodeDetails := range e.PluginNodeDetails {
		c, err := client.NewPluginClient(&client.PluginConfig{
