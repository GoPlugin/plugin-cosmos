import { CosmosCommand, logger } from '@plugin/gauntlet-cosmos'
import { Middleware, Next, AddressBook } from '@plugin/gauntlet-core'
...
export const withAddressBook: Middleware = async (c: CosmosCommand, next: Next) => {
  ...
  if (!addressBooks.has(chainId)) {
    ...
    tryAddInstance(CONTRACT_LIST.CW20_BASE, process.env['PLI'], 'link')
