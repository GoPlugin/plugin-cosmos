import { BN, inspection, logger, longs } from '@plugin/gauntlet-core/dist/utils'
import { RDD, Client } from '@plugin/gauntlet-cosmos'
...
const makeInput = async (flags: any, args: string[]): Promise<ContractExpectedInfo> => {
  ...
  if (flags.rdd) {
    ...
    const link = flags.link || process.env.PLI
  ...
  return {
    ...
    link: flags.link || process.env.PLI,
    billing: {
...
const inspect = (expected: ContractExpectedInfo, onchainData: ContractExpectedInfo): boolean => {
  let inspections: inspection.Inspection[] = [
    ...
    inspection.makeInspection(onchainData.link, expected.link, 'PLI'),
    inspection.makeInspection(
  ...
  `)
  logger.info(`Funding:
    - PLI Available: ${onchainData.linkAvailable}
    - Total PLI Owed: ${onchainData.totalOwed}
  `)
  ...
  if (owedDiff.lt(new BN(0))) {
    logger.warn(`Total PLI Owed is higher than balance. Amount to fund: ${owedDiff.mul(new BN(-1)).toString()}`)
  } else {
    logger.success(`PLI Balance can cover debt. PLI after payment: ${owedDiff.toString()}`)
  }
